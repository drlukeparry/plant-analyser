...
**Not yet done:** since species identity is ruled out as the main driver, the next candidate explanations are per-image imaging/segmentation variation (stain intensity, resolution, `segment_classical` parameter sensitivity across very different cell densities) or the earlier-flagged raw-pixel-scale pooling artifact revisited at the per-specimen rather than per-species level; the optional auxiliary tissue-type classification head; reconstruction-quality metrics beyond the training loss curve.
