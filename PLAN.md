...
**Not yet done:** distinguishing genuine cross-dataset biological signal from an unwanted imaging/domain-shift artifact in that remaining ~0.25 variance gap (candidate next step: per-image z-scoring before pooling, or an explicit domain-adversarial term if the model should be invariant to source dataset); confirming `EH`/`VM` species identity (Phase 1 open question) would help interpret which explanation is more likely; the optional auxiliary tissue-type classification head; reconstruction-quality metrics beyond the training loss curve.
