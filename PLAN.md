...
**Not yet done:** isolating whether the 0.387 source-image variance is a real cross-specimen effect or a scale-normalization artifact (try per-image z-scoring before pooling, or restrict the batch run to `DO` only as a control since those are same-stain/species); the optional auxiliary tissue-type classification head; reconstruction-quality metrics beyond the training loss curve.
